...
echo "🧪 Testing Math API with Image Upload (with Subscription)\n";
echo "========================================================\n\n";
...
if ($httpCode === 200) {
    ...
    if ($responseData) {
        ...
        if (isset($responseData['math_problem']['problem_image'])) {
            $imagePath = storage_path('app/public/' . $responseData['math_problem']['problem_image']);
            if (file_exists($imagePath)) {
                echo "✅ Image stored successfully at: $imagePath\n";
            } else {
